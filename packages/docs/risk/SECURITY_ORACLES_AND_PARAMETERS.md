...
**Related:** [RISKS_GAPS_AND_GAMIFICATION.md](RISKS_GAPS_AND_GAMIFICATION.md) (attack vectors), [REMEDIATION_AND_STAYING_AHEAD.md](REMEDIATION_AND_STAYING_AHEAD.md) (resolution matrix), [RISK_COMMITTEE_CHARTER.md](RISK_COMMITTEE_CHARTER.md) (governance).
...
- **Timelock:** All critical oracle and risk param changes (e.g. `setPriceFeed`, `setTokenPriceFeed`, `setMaxPriceAge`, `setVolatility`, LTV-related) go through queue/execute with a delay (e.g. ≥24h mainnet). No bypass for “routine” updates ([SECURITY.md](../security/SECURITY.md)).
...
- **Parameters:** LTV cap, max borrow, unlock-impact discount, volatility, liquidation minOut, per-token eligibility, **per-pool and per-token concentration caps** (max exposure to one token / unlock window; see `docs/risk/VESTED_TOKEN_LENDING_SESSION_2026-02-14.md`), and (when used) price history (ATH/ATL) and drawdown caps bound the impact of price manipulation and illiquid default. See RISK_MODELS.md § Price history for ATH/ATL usage.
...
- **Staying ahead:** Run the red-team checklist on new assets and param changes; keep [RISKS_GAPS_AND_GAMIFICATION.md](RISKS_GAPS_AND_GAMIFICATION.md) and [REMEDIATION_AND_STAYING_AHEAD.md](REMEDIATION_AND_STAYING_AHEAD.md) updated and linked from this doc and the Risk Committee Charter.
