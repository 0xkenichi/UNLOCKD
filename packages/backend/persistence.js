...
const createLoan = async ({
  ...
  if (collateralItems && collateralItems.length > 0) {
    const collateralStmt = sqlite.prepare(`
      INSERT INTO loan_collateral (id, loan_id, source_id, collateral_amount, raw_data, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
...
const getIdentityProfileByWallet = async (walletAddress) => {
  ...
  if (useSupabase) {
    ...
    if (error) {
      // If table doesn't exist, log warning and return null (allow fallback or empty state)
      if (error.code === '42P01' || error.message?.includes('schema cache')) {
        console.warn(`[supabase] table 'identity_profiles' missing, returning empty profile`);
        return null;
      }
...
const listIdentityAttestations = async (walletAddress) => {
  ...
  if (useSupabase) {
    ...
    if (error) {
      if (error.code === '42P01' || error.message?.includes('schema cache')) {
        console.warn(`[supabase] table 'identity_attestations' missing, returning empty array`);
        return [];
      }
