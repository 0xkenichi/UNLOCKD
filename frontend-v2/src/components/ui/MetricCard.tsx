...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  return (
    <Card className={`relative overflow-hidden ${glowStyles[glowColor]}`}>
      ...
      {glowColor !== 'none' && (
        <div className={`absolute -right-4 -bottom-4 w-24 h-24 blur-[40px] opacity-20 rounded-full ${glowColor === 'teal' ? 'bg-accent-teal' : 'bg-accent-cyan'}`} />
      )}
    </Card>
  );
};
