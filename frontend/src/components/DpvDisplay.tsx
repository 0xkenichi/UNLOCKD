...
export function DpvDisplay({
  ...
  const dpvWad  = (dpvResult as any)?.[0] ?? 0n;
  const ltvBps  = (dpvResult as any)?.[1] ?? 0n;
    ...
    <GlassCard className="border-[#1D9E75]/20">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          <InfoRow
            label="Borrowed Rate (Net)"
            value={`${(finalAprBps / 100).toFixed(2)}%`}
            status="success"
          />
          <InfoRow
            label="Borrow Limit (LTV Cap)"
