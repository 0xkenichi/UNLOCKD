...
export default async function DocsPage() {
  // Robustly find root dir: check if packages/docs exists in current or parent
  const currentDir = process.cwd();
  let rootDir = currentDir;
  
  if (!fs.existsSync(path.join(rootDir, "packages/docs"))) {
    rootDir = path.join(currentDir, "..");
    if (!fs.existsSync(path.join(rootDir, "packages/docs"))) {
      rootDir = path.join(currentDir, "../../"); // Deep fallback
    }
  }
