        ...
        <div className="p-10 rounded-[2.5rem] bg-white/[0.02] border border-white/5">
          ...
          <button 
            onClick={async () => {
              if (!address) return;
              try {
                toast.loading("Syncing passport...", { id: "sync" });
                await import("@/utils/api").then(m => m.api.verifyIdentity(address));
                toast.success("Passport synced successfully!", { id: "sync" });
              } catch (e) {
                toast.error("Failed to sync passport", { id: "sync" });
              }
            }}
        ...
        <div className="space-y-8">
          <div className="p-10 rounded-[2.5rem] bg-white/[0.02] border border-white/5">
            ...
            <div className="space-y-8">
              {[
                { 
                  label: "Trust Protocol Baseline", 
                  score: 100, 
                  max: 100, 
                  icon: Shield, 
                  color: "accent-teal/50", 
                  desc: "Baseline credit establishment score provided to all verified protocols." 
                },
                { 
                  ...
                  icon: Fingerprint, 
                  color: "accent-teal", 
                  desc: "Gitcoin Passport points calculated on-chain." 
                },
                { 
                  ...
                  icon: Activity, 
                  color: "accent-cyan", 
                  desc: `Active: ${passport.activityMetrics?.latestTxTimestamp ? new Date(passport.activityMetrics.latestTxTimestamp * 1000).toLocaleDateString() : 'Never'} | Trx: ${passport.activityMetrics?.txCount || 0} (${((passport.activityMetrics?.txCount || 0) / Math.max(1, (passport.activityMetrics?.ageMonths || 1))).toFixed(1)}/mo) | Vol: $${Math.floor(passport.activityMetrics?.totalVolume || 0).toLocaleString()} | Max Tx: $${Math.floor(passport.activityMetrics?.largestTx || 0).toLocaleString()}` 
                },
                { 
