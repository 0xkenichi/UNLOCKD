 ...
 *   // results: Array<{ publicKey: PublicKey; account: Stream }>
 *   // Stream: { mint, depositedAmount, end, cliff, cliffAmount, canceledAt, period, name }
 * ─────────────────────────────────────────────────────────────────────────────
 */
  ...
  private async fetchStreamflowPositions(wallet: string): Promise<VestingPosition[]> {
    ...
    for (const rpcUrl of SOLANA_RPC_ENDPOINTS) {
      try {
        ...
        // searchStreams returns { publicKey: PublicKey; account: Stream }[]
        const results: Array<{
