...
export function useLoan(): UseLoanReturn {
  ...
  const borrow = useCallback(async (params: BorrowParams) => {
    ...
    try {
      ...
      const openedLog = receipt.logs.find((log) =>
        log.topics[0] === '0x' + keccak256Hex('LoanOpened(uint256,address,address,uint256,uint256,uint256,uint256,uint256)')
      );
      if (openedLog) {
        // loanId is topics[1]
...
// Minimal keccak256 event sig helper (replace with viem's keccak256 in prod)
function keccak256Hex(sig: string): string {
  // In production import: import { keccak256, toHex } from 'viem'
  // This placeholder makes the file self-contained for review
  return 'placeholder_use_viem_keccak256';
}
