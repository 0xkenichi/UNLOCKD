...
### 1.6 Founder / Insider and Wallet–Token Risk

| Risk | Description | Gamification / Cheat |
|------|-------------|----------------------|
| **Insider information and price control** | Founders/team know when their token may dump and can time borrows and repayments. If we don’t know the counterparty is an insider, we treat them like any user. | Pump token (liquidity, buy, coordinate) → borrow at high DPV → at repay dump price → protocol liquidates at a loss or is left with low-value collateral; insider extracts loan value. |
| **No wallet–token relationship view** | We do not yet have an internal “bubble map”: wallet ↔ token (founder/team?), wallet ↔ vesting type, wallet ↔ other holders. | Multiple founders of same protocol exit via loans; we don’t flag coordinated exit or apply insider-aware LTV/caps. |
| **Privacy vs risk scrutiny** | External privacy (hide escrow/loan from third parties) must not block internal risk scrutiny. | If we don’t scrutinize internally, we cannot flag insider or cohort behavior while still preserving user privacy. |

**Gaps:** No systematic flagging of founder/team/insider wallets per token; no cohort alert when multiple wallets tied to same protocol take loans; no documented insider-aware LTV or caps. **Detail:** `docs/FOUNDER_INSIDER_RISK_AND_FLAGGING.md`.

---

## 2. Identity, Scoring, and Eligibility (Gamification Hotspots)

### 2.1 Passport / Identity Score (Backend)
...
11. **Founder/insider pump–dump** – Insider pumps token → borrows at high DPV → dumps at repay → we liquidate at a loss; we may not know counterparty is founder/team. Mitigation: internal wallet–token relationship map and flagging; insider-aware LTV/caps; see **`docs/FOUNDER_INSIDER_RISK_AND_FLAGGING.md`**.
...
- **Medium**  
  ...
  - Implement internal risk-intel and flagging: wallet–token relationship (founder/team/insider), vesting type/source, cohort detection (multiple same-protocol exits); insider-aware LTV or caps for flagged counterparties; see **`docs/FOUNDER_INSIDER_RISK_AND_FLAGGING.md`**.
